...
const props = defineProps<{
    ...
    const checkIfAllowWeekends = computed(function () {
      //return false;
      if (usePage().props.auth.user.permissions.admin) return true;
    ...
    const canEmailAll = computed(function () {
      ...
      if (usePage().props.auth.user.permissions.admin && sum && haveEmail)
        return true;
      return false;
    });
    ...
    const canEmail = computed(function () {
      if (usePage().props.auth.user.permissions.admin) return true;
      if (usePage().props.auth.user.permissions.teacher && props.allowTeachersEmail == 1 && props.isToday ) return true;
      return false;
    });

    function apouSubmit() {
      apouForm.post(
        route("apousies.store") + `/${props.selectedTmima}/${props.date}`,
        { preserveScroll: true }
      );
    }
    ...
    function checkCol(index) {
      ...
      props.arrStudents.forEach((student) => {
        if(props.arrApousies[student.id]['teach'][index] !== '' && !(props.arrApousies[student.id]['teach'][index] == usePage().props.auth.user.id)) {
          chkNotSameTeacher =  true
        }
      });
      if (usePage().props.auth.user.permissions.admin) chkNotSameTeacher = false
    ...
    function checkRow(id) {
      // μόνο ο Διαχειριστής
      if (!usePage().props.auth.user.permissions.admin) return;
      let newValue;
      for (let loopIndex in apouForm[id]['apou']) {
        ...
        apouForm[id]['teach'][loopIndex] = usePage().props.auth.user.id;
        if(newValue == false) {
    ...
    function disableDatePicker(){
      if(usePage().props.auth.user.permissions.admin) return false
    ...
    function checkDisabled(studentId,index){
      ...
      if(varHoursUnlocked.value && (!props.arrApousies[studentId]['teach'][index] || usePage().props.auth.user.id == props.arrApousies[studentId]['teach'][index])) return false
      return true
    }
    ...
    function toggleApovoli( studentId, index){
      ...
      if(apouForm[studentId]['apov'][index]==false) {
        ...
        apouForm[studentId]['teach'][index]=usePage().props.auth.user.id
      }else{
    ...
    // παρακολουθεί όταν αλλάζει η ημερομηνία 
    // και ξαναφορτώνει τη σελίδα με τη νέα ημερομηνία
    watch(
      () => apouForm.date, // use a getter like this
      () => router.get(route('apousies', {t: props.selectedTmima, d: apouForm.date}))
    )
...
<template>
  ...
  <AppLayout>
    ...
    <GthSuccess property="saveSuccess">{{ printTime() }}</GthSuccess>
          ...
          <div class="p-4 bg-white border-b border-gray-200 space-y-4">
            ...
            >
              ...
              <Link
                ...
                :key="anathesi"
                :href="route('apousies', {t: anathesi, d: date})"
                class="mr-2"
                :class="{
              ...
              <Link
                ...
                :href="route('apousies', {t:'0', d:date})"
                class="text-xl"
                :class="{
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    <!-- κουμπί αποβολής -->
                    <div 
                      class="h-2 bg-gray-300 mt-0.5 ml-1 mr-1 cursor-pointer rounded" 
                      title="Αποβολή" 
