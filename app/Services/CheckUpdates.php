...
class CheckUpdates
{
    ...
    public function chkForUpdates()
    {
        ...
        $gitRepo = 'g-theodoroy/apousiologos-v2';

        try {
            // έλεγχος εάν έχουν γίνει αλλαγές στο github
            $url = "https://api.github.com/repos/$gitRepo/commits";
            ...
            if (Auth::user()->permissions['admin']) {
                ...
                if (file_exists($file)) {
                    ...
                    if ($commits[0]['sha'] != file_get_contents($file)) {
                        ...
                        $message .= 'Αν επιθυμείτε <a href="https://github.com/' . $gitRepo . '/commits/" target="_blank"><u>εξετάστε τον κώδικα</u></a> και ενημερώστε την εγκατάστασή σας.<br><br>';
                        $message .= 'Για να μην εμφανίζεται το παρόν μήνυμα καντε κλικ εδώ: <a href="setUpdated/' . $commits[0]['sha'] . '" >"<u>Ενημερώθηκε</u>"</a>';
                        return redirect()->route('apousies')->with(['message' => ['checkUpdates' => $message]]);
                    }
                } else {
